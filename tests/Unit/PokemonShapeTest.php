...
beforeEach(function (): void {
    $this->endpoint = ResourceEndpoints::POKEMON_SHAPE;
    ...
    $this->singleRequestClass = GetSinglePokemonShape::class;
    $this->allRequestClass = GetAllPokemonShapes::class;
...
describe('Get a single Pokemon Shape', function (): void {
    it('sends a request with an id (int) and receives the expected response', function (): void {
        $response = $this->connector->pokemonShape()->get(1);
    ...
    it('sends a request with a name (string) and receives the expected response', function (): void {
        ...
        $response = $this->connector->pokemonShape()->get('ball');
...
describe('Get all Pokemon Shape', function (): void {
    it('sends a request and receives the expected response', function (): void {
        $response = $this->connector->pokemonShape()->all();
