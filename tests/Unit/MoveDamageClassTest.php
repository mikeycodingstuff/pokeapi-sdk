...
beforeEach(function (): void {
    $this->endpoint = ResourceEndpoints::MOVE_DAMAGE_CLASS;
    ...
    $this->singleRequestClass = GetSingleMoveDamageClass::class;
    $this->allRequestClass = GetAllMoveDamageClasses::class;
...
describe('Get a single Move DamageClass', function (): void {
    it('sends a request with an id (int) and receives the expected response', function (): void {
        $response = $this->connector->moveDamageClass()->get(1);
    ...
    it('sends a request with a name (string) and receives the expected response', function (): void {
        ...
        $response = $this->connector->moveDamageClass()->get('status');
...
describe('Get all Move DamageClasses', function (): void {
    it('sends a request and receives the expected response', function (): void {
        $response = $this->connector->moveDamageClass()->all();
